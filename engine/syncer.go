	...
	for x := range e.CurrencyPairs {
		if e.CurrencyPairs[x].Exchange == exchangeName &&
			...
			case SyncItemTicker:
				...
				log.Debugf("%s ticker sync complete %v", exchangeName, p)
			...
			case SyncItemOrderbook:
				...
				log.Debugf("%s orderbook sync complete %v", exchangeName, p)
			...
			case SyncItemTrade:
				...
				log.Debugf("%s trade sync complete %v", exchangeName, p)
			}
		}
	}
}
						...
						if !e.isProcessing(exchangeName, c.Pair, c.AssetType, SyncItemTicker) {
							if c.Ticker.LastUpdated.IsZero() {
								if c.Ticker.IsUsingWebsocket {
									if time.Since(c.Created) < time.Second*15 {
										continue
									}
								...
								if c.Ticker.IsUsingREST {
									if !c.Ticker.LastUpdated.IsZero() && time.Since(c.Orderbook.LastUpdated) < time.Second*10 {
										continue
									}
									...
									if supportsRESTTickerBatching {
										...
										if batchLastDone.IsZero() || time.Since(batchLastDone) > time.Second*10 {
											e.mux.Lock()
					...
					if e.Cfg.SyncOrderbook {
							...
							if c.Orderbook.LastUpdated.IsZero() {
								if c.Orderbook.IsUsingWebsocket {
									...
									if supportsREST {
										...
										log.Warnf("%s %s: No orderbook update after 10 seconds, switching from websocket to rest",
											c.Exchange, c.Pair.Pair())
										e.setProcessing(c.Exchange, c.Pair, c.AssetType, SyncItemOrderbook, false)
									}
								}

								if c.Orderbook.LastUpdated.IsZero() && time.Since(c.Orderbook.LastUpdated) < time.Second*10 {
									continue
								}
						...
						if e.Cfg.SyncTrades {
							if !e.isProcessing(exchangeName, c.Pair, c.AssetType, SyncItemTrade) {
								if c.Trade.LastUpdated.IsZero() || time.Since(c.Trade.LastUpdated) > time.Second*10 {
