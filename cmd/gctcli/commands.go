...
func addPortfolioAddress(c *cli.Context) error {
	...
	if c.IsSet("balance") {
		...
		balance, _ = strconv.ParseFloat(c.Args().Get(4), 64)
	}
...
func submitOrder(c *cli.Context) error {
	...
	if c.IsSet("amount") {
		...
		amount, _ = strconv.ParseFloat(c.Args().Get(5), 64)
	}

	if c.IsSet("price") {
		price = c.Float64("price")
	} else {
		price, _ = strconv.ParseFloat(c.Args().Get(6), 64)
	}

	if c.IsSet("client_id") {
		clientID = c.String("client_id")
	} else {
		clientID = c.Args().Get(7)
	}
...
func cancelOrder(c *cli.Context) error {
	...
	if c.IsSet("wallet_address") {
		...
		walletAddress = c.Args().Get(6)
	}

	if c.IsSet("order_side") {
		orderSide = c.String("order_side")
	} else {
		orderSide = c.Args().Get(7)
	}
